...
class DQN():
    ...
    def update(self, total_step):
        ...
        s_prev_replay_84 = np.ndarray(shape=(self.batch_size, 4, 84, 84), dtype=np.float32)
        a_replay = np.ndarray(shape=(self.batch_size, 1), dtype=np.uint8)
        r_replay = np.ndarray(shape=(self.batch_size, 1), dtype=np.float32)
        s_replay_84 = np.ndarray(shape=(self.batch_size, 4, 84, 84), dtype=np.float32)
        done_replay = np.ndarray(shape=(self.batch_size, 1), dtype=np.bool)

        for i in xrange(self.batch_size):
            s_prev_replay_84[i] = np.asarray(self.replay_memory[0][index[i]], dtype=np.float32)
            a_replay[i] = self.replay_memory[1][index[i]]
            r_replay[i] = self.replay_memory[2][index[i]]
            s_replay_84[i] = np.array(self.replay_memory[3][index[i]], dtype=np.float32)
            done_replay[i] = self.replay_memory[4][index[i]]

        s_prev_replay = np.fft.fft2(s_prev_replay_84, [28,28]).reshape(32,4*28*28).real
        s_replay = np.fft.fft2(s_replay_84, [28,28]).reshape(32,4*28*28).real
        s_prev_replay = np.asarray(s_prev_replay, dtype=np.float32)
        s_replay = np.asarray(s_replay, dtype=np.float32)
    ...
    def epsilon_greedy(self, s_84, epsilon):
        s = np.fft.fft2(s_84, [28,28]).reshape(1,4*28*28).real
