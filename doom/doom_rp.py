...
class DQN():
    ...
    def update(self, total_step):
        ...
        s_rp_prev_replay = self.RP32(s_prev_replay)
        s_rp_replay = self.RP32(s_replay)

        if self.gpu >= 0:
            s_rp_prev_replay = cuda.to_gpu(s_rp_prev_replay, device=gpu)
            s_rp_replay = cuda.to_gpu(s_rp_replay, device=gpu)

        self.model.zerograds()
        loss = self.compute_loss(s_rp_prev_replay, a_replay, r_replay, s_rp_replay, done_replay)
        #print loss.data
        loss.backward()
    ...
    def epsilon_greedy(self, s_rp, epsilon):
        s_rp = np.asarray(s_rp.reshape(1, 3136), dtype=np.float32)
        if self.gpu >= 0:
            s_rp = cuda.to_gpu(s_rp, device=gpu)
        s_rp = Variable(s_rp)
    ...
    def RP(self, s):
        s = s.reshape(84*84*4, 1)
     	s_rp = cuda.cupy.dot(random_matrix_gpu, cuda.cupy.asarray(s))
        s_rp = cuda.to_cpu(s_rp)
     	return s_rp

    def RP32(self, s):
        s = s.reshape(32, 84*84*4)
        s_rp = cuda.cupy.dot(cuda.cupy.asarray(s),random_matrix_gpu_T)
        s_rp = cuda.to_cpu(s_rp)
        return s_rp
...
random_matrix = np.random.randn(3136, 84*84*4)
random_matrix = np.array(random_matrix, dtype=np.float32)
random_matrix_gpu = cuda.to_gpu(random_matrix)
random_matrix_gpu_T = cuda.to_gpu(random_matrix.T)
def evaluation():
    global env_eval
    global eval_counter
    ...
    for eval_episode in range(n_episode):
        ...
        while (True):
            ...
            if randomskip == 0:
                reward = 0
                s_rp = dqn.RP(s)
                a = dqn.epsilon_greedy(s_rp, eval_epsilon)
