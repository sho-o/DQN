...
class Agent():
	def __init__(self, exp_policy, net_type, gpu, pic_size, num_of_actions, memory_size, input_slides, batch_size, discount, rms_eps, rms_lr, optimizer_type, mode, threshold, penalty_weight, mix_rate, penalty_function, penalty_type, final_penalty_cut):
		...
		if self.net_type == "grad":
			self.q = network.Grad_test(self.num_of_actions)
		self.fixed_q = copy.deepcopy(self.q)
		self.replay_memory = {"s":np.zeros((self.memory_size, self.input_slides, self.size), dtype=np.float32),
							"a":np.zeros(self.memory_size, dtype=np.int),
							"r":np.zeros((self.memory_size), dtype=np.float32),
							"new_s":np.zeros((self.memory_size, self.input_slides, self.size), dtype=np.float32),
		...
		if self.optimizer_type == "sgd":
			self.optimizer = optimizers.SGD(lr=rms_lr)
	...
	def policy(self, s, eva=False):
		if self.net_type == "full" or self.net_type == "grad":
			s = np.asarray(s.reshape(1, self.input_slides*self.size), dtype=np.float32)
		...
		if self.exp_policy == "epsilon_greedy":
			...
			if np.random.rand() < epsilon:
				...
				#print action, "agt_action"
				#print num_of_actions, "agt_num_a"
			else:
	...
	def make_minibatch(self, total_step):
		...
		s_batch = np.ndarray(shape=(self.batch_size, self.input_slides, self.size), dtype=np.float32)
		a_batch = np.ndarray(shape=(self.batch_size), dtype=np.int)
		r_batch = np.ndarray(shape=(self.batch_size), dtype=np.float32)
		new_s_batch = np.ndarray(shape=(self.batch_size, self.input_slides, self.size), dtype=np.float32)
	...
	def compute_loss(self, s, a, r, new_s, done, loss_log=False):
		if self.net_type == "full" or self.net_type == "grad":
			s = s.reshape(self.batch_size, self.input_slides*self.size)
			new_s = new_s.reshape(self.batch_size, self.input_slides*self.size)
