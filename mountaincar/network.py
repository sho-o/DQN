...
class Q(Chain):
	def __init__(self, num_of_actions, n_units=100):
...
class Q_conv(Chain):
	def __init__(self, num_of_actions, n_units=1000):
		super(Q_conv, self).__init__(
			conv1=L.Convolution2D(None, 20, ksize=5, stride=2),
			conv2=L.Convolution2D(None, 50, ksize=5, stride=2),
			l1=L.Linear(None, 500),
			l2=L.Linear(None, num_of_actions))

	def __call__(self, x):
		h_1 = F.relu(self.conv1(x / 255.0))
...
class Grad_test(Chain):
	def __init__(self, num_of_actions):
		super(Grad_test, self).__init__(
			l1=L.Linear(None, num_of_actions, initialW=np.zeros((num_of_actions, 3),dtype=np.float32)))
