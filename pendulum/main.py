...
parser.add_argument('--game', '-G', default='pendulum', choices=['atari', 'doom', 'pendulum'], type=str, help='game type (doom or atari)')
...
parser.add_argument('--net_type', '-n', type=str, default="full", help='network type')
parser.add_argument('--pic_size', '-ps', type=int, default=3, help='nput pic size')
...
parser.add_argument('--eval_iter', '-ei', type=int, default=100, help='iteration of evaluation')
...
def run(args):
	...
	if game == "pendulum":
		...
		num_of_actions = len(pendulum_actions)
	run_start = time.time()
	...
	for episode in range(max_episode):
		...
		for steps in range(max_step):
			...
			agt.store_experience(total_step, s, a, r, new_s, done)
